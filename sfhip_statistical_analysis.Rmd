...
opts_chunk$set(fig.width=6, fig.height=5, dpi=120, warning=F, echo=F,
               cache=T, message=F)
```

```{r theme}
custom_theme <- function(text_size=6, legend_size=0.4, margin_size=0.01) {
    # Custom ggplot theme
...
# Alcohol crime cols
alc_laws_cors <- ProcessCors(crime_census_alcohol[, c(census_cols, alcohol_crime_cols, license_cols)])
...
cancor_result <- cancor(scale(crime_census_alcohol[, license_cols]), 
                        scale(crime_census_alcohol[, alcohol_crime_cols]))
cancor_plot_data <- rbind(data.frame(cancor_result$xcoef[, 1:2], type="license", name=gsub("license_", "", license_cols)),
                          data.frame(cancor_result$ycoef[, 1:2], type="crime", name=gsub("crime_", "", alcohol_crime_cols)))
ggplot(cancor_plot_data) + geom_text(aes(x=X1,y=X2,label=name))

cancor_result <- cancor(scale(crime_census_alcohol[, license_cols]), 
                        scale(crime_census_alcohol[, crime_cols]))
cancor_plot_data <- rbind(data.frame(cancor_result$xcoef[, 1:2], type="license", name=gsub("license_", "", license_cols)),
                          data.frame(cancor_result$ycoef[, 1:2], type="crime", name=gsub("crime_", "", crime_cols)))
...
### Crime
- What are the tracts that have the highest overall crime, liquor related crime
